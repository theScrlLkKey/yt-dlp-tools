...
# show new members - liveChatTickerSponsorItemRenderer - done
# what is placeholderitem
        ...
        with open(f"test/{video_id}.live_chat.json", encoding="utf8") as file:
            for line in file:
                try:
                    ...
                    # normal or donation
                    # turns out liveChatTickerPaidMessage just duplicates some liveChatPaidMessage
                    message_parsing = chat_message["replayChatItemAction"]["actions"][0]["addChatItemAction"]["item"]
                    # try:
                    message_parsing = message_parsing.get("liveChatTextMessageRenderer") or message_parsing.get("liveChatPaidMessageRenderer") or message_parsing.get("liveChatMembershipItemRenderer")
                    # except KeyError:
                        # message_parsing = chat_message["replayChatItemAction"]["actions"][0]["addLiveChatTickerItemAction"]["item"]["liveChatTickerPaidMessageItemRenderer"]["showItemEndpoint"]["showLiveChatItemEndpoint"]["renderer"].get("liveChatPaidMessageRenderer")
                    ...
                    # easy member parsing (really stupid)
                    try:
                        message_parsing["member"] = str(message_parsing["headerSubtext"]["runs"][0]["text"]) + ":"
                    except KeyError:
                        message_parsing["member"] = ""
...
def display_content(seconds):
    ...
    for index, chat_message in enumerate(chat_data):
        ...
        member = chat_message.get("member", "")
        # it must have these
        ...
        # pure donos/members have no message
        ...
        if chat_timestamp <= current_time:
            messages_to_print.append((timestamp_text, member, paid_amount, author_name, message_text))
        else:
            break

    # Print chat messages, then remove them
    for timestamp_text, member, paid_amount, author_name, message_text in messages_to_print:
        print(f"{paid_amount}{member} [{timestamp_text} | {author_name}] {message_text}")
